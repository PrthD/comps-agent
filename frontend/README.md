...
(`VITE_API_BASE_URL`) and is built for static hosting on Vercel (P6).
...
On Vercel, set `VITE_API_BASE_URL` to the Render backend URL (P6).
...
- **Cold start:** the backend (Render free tier) sleeps after ~15 min and wakes in ~30s. On load the
  app pings `/api/health` and shows a "warming up" state with auto-retry, gating the input UI on the
  engine being ready — never a raw failed fetch.
